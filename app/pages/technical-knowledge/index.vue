<template>
  <MainCategory
      name="category"
      :blogs="updateBlogList"
      :loading="blogLoadingList"
      :categories="categoryItems"
      :active-category-id="activeTab"
      :subcategories="currentSubcategories"
      :active-tab="activeCategory"
      @updateCategory="getBlog"
      @updateSubcategory="selectSubcategory"
      primary-color="#276ef0"

      ref="sect"
      v-show="showSection"
      class="invisible"
      title="همه دسته بندی های تخصصی "
  >
    <template #title>
      دانشنامه فنی
    </template>
...
<!--        <div class="border-t border-b mt-8 md:mt-0">
        ...
        <div v-if="isLoading" class="flex w-full h-24 my-auto items-center justify-center text-center">
          <div class="text-center flex items-center justify-center flex-col gap-y-2">
            <div role="status">
              <svg aria-hidden="true" class="w-8 h-8 text-gray-200 animate-spin fill-black"
        ...
        <div v-else>
          <div v-if="updateBlogList && updateBlogList.length> 0"
               v-for="(item, index) in updateBlogList"
               :key="index"
               class="flex w-full relative">
            <nuxt-link :to="`/technical-knowledge/${selectedSlug}/${item.id}/${encodeURIComponent(item.slug)}`"
                       class="relative overflow-hidden inline-block text-[#1e1e1e] group py-4 w-full">
              <div class="flex items-center w-full relative gap-x-4">
                <div class="w-full flex flex-col py-2">
                  ...
                  <h3 class="font-medium text-medium lg:mb-0 mb-2 truncate text-[#276ef0]" v-html="seperatedText(item.title)"></h3>
                  <div
                      class="mb-2 text-sm text-gray-500 leading-relaxed lg:min-h-[80px] lg:max-h-[90px] overflow-hidden">
                      <span class="line-clamp-3 pt-2" v-if="item.short_desc"
                            v-html="seperatedText(item.short_desc.slice(0, 250))"></span>
                  </div>
                  <div class="flex items-center justify-between w-full">
                    <p class="w-full text-xs text-gray-400 mt-2 text-right">
<!--                      نوشته شده توسط
                      {{ item.author }}
                      &nbsp;|&nbsp;-->
                      {{ formatJalaliDate(item.created_at) }}
                    </p>
                    <p class="w-full text-xs text-[#276ef0] mt-2 text-left">
                      ادامه مطلب
                    </p>
                  </div>
              ...
              <span
                    class="absolute bottom-0 right-0  w-full h-px bg-black opacity-20"></span>
              <span

                  class="absolute bottom-0  h-px w-full bg-black transition-all duration-700 left-[calc(100%-10px)] group-hover:left-0 "></span>
...
const getBlog = async (item, type) => {
  ...
  if(type === 'category'){
    await getData(activeCategory.value.id)
  }
  if(type === 'subcategory'){
    await getData(activeSubCategory.value.id)
  }
...
if (response.value) {
  ...
  useSeoMeta({
    title: 'دانشنامه فنی',
    ogTitle: 'دانشنامه فنی',
    description: 'دانشنامه فنی',
    ogDescription: 'دانشنامه فنی',
  })
...
async function getData(id) {
  ...
  try {
    ...
    updateBlogList.value = [...data.data]

    const currentLength = blogList.value.length;
