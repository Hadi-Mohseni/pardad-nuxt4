    ...
    <div class="h-full" ref="el">
      ...
      <div v-else>
        <div v-if="updateBlogList && updateBlogList.length > 0"
             ref="bodyElement"
             class="h-full grid grid-cols-1 lg:grid-cols-2 gap-x-6 w-full">
          ...
          >
            <nuxt-link :title="item.title"
                       ...
                       :to="`/blog/${route.params.category_name}/${item.id}/${encodeURIComponent(item.slug)}`"
            >
...
async function getData(id) {
  ...
  try {
    const data = await $fetch(`${config.public.apiUrl}/product/${product.value.code}/blogs`, {
      query: {
        locale: 'fa',
        page: page.value,
