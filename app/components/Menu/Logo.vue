...
watch([getLoading, getMenuStatus , getCurrentSlide, () => route.name], () => {
  if (process.client) {
    animateLogoDebounced()
  }
})
...
const isMobile = () => {
  if (process.client) {
    return window.innerWidth <= 768 // هر چی خواستی تغییر بده
  }
  return false
}
